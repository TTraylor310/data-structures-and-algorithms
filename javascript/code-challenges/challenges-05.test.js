...
const toLastNames = people => {
  let newArray = [];
  people.map( n => {
    newArray.push(n.firstName+' '+n.lastName);
  });
  return newArray;
};
...
const countNumberOfElements = (arr) => {
  let ans = arr.reduce( (acc) => acc+1, 0);
  return ans;
};
...
const reversedString = (str) => {
  let sums = str.split('').reduce ( (rev, char) => char + rev, '');
  return sums;
};

// saw few different places, used from "https://stackoverflow.com/questions/958908/how-do-you-reverse-a-string-in-place-in-javascript"
